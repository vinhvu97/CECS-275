...
static const string ERROR_STRING = "String not found";
static const string ERROR_INDEX = "Index out of range";
static const string ERROR_EMPTY = "List is empty";
...
{
	if (index<0 || index>getCount() - 1) throw ERROR_INDEX;
	int ind = 0;
...
{
	if (!isPresent(elementToAddBefore)) throw ERROR_STRING;
...
{
	if (!isPresent(elementToAddAfter)) throw ERROR_STRING;
...
{
	if (!isPresent(element)) throw ERROR_STRING;
